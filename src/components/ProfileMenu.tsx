    ...
    <div className="me-wrap" ref={wrap}>
      ...
      {open && (
        <div className="me-menu" role="menu" aria-label="Profile" onKeyDown={onMenuKey}>
          ...
          {!acc.demo && <button role="menuitem" className="menu-item" onClick={() => { setOpen(false); setPinOpen(true); }}>{Icon.lock}<span>{acc.pinHash ? "Change PIN" : "Add a PIN"}<small>{acc.pinHash ? "Update your 4-digit PIN" : "Lock your profile on this laptop"}</small></span></button>}
...
export function PinSheet({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    if (step === "confirm" && p2.length === 4) {
      ...
      else setPin(acc.id, p1).then(() => { toast("PIN saved"); onClose(); });
    }
  ...
  return (
    <Sheet open={open} onClose={onClose} label="Change PIN" title={<div className="spread"><h2>{acc.pinHash ? "Change PIN" : "Add a PIN"}</h2><button className="icon-btn" onClick={onClose} aria-label="Close">{Icon.x}</button></div>}>
      <div className="pin-step" style={{ padding: "8px 0 12px" }}>
        ...
        {acc.pinHash && step !== "old" && (
          <button className="btn ghost sm" onClick={async () => { await setPin(acc.id, ""); toast("PIN removed"); onClose(); }}>Remove my PIN instead</button>
        )}
      </div>
    </Sheet>
  );
