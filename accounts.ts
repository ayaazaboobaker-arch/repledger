...
/** Who's signed in lives in sessionStorage: it survives a refresh, but is wiped when the tab or app is closed. */
...
function load(): AccountsState {
  ...
  if (saved && Array.isArray(saved.accounts)) return { accounts: saved.accounts, currentId: null };
  return { accounts: [], currentId: null };
}

export const useAccounts = create<AccountsState>()(() => load());
/** Re-read profiles from storage (after restoring a backup). */
export const reloadAccounts = () => useAccounts.setState({ accounts: load().accounts });
useAccounts.subscribe((s) => { store.set(KEY, JSON.stringify({ accounts: s.accounts, currentId: null })); session.set(s.currentId); });

const touch = (id: string) =>
  useAccounts.setState((s) => ({ currentId: id, accounts: s.accounts.map((a) => (a.id === id ? { ...a, lastUsed: Date.now() } : a)) }));

/** Stay signed in across a refresh, but ask again (profile + PIN) once the app has been closed. */
export function restoreSession() {
  const id = session.get();
  const { accounts } = useAccounts.getState();
  if (id && accounts.some((a) => a.id === id)) { openUserData(id); useAccounts.setState({ currentId: id }); keepData(); }
  else { session.set(null); useAccounts.setState({ currentId: null }); }
}
